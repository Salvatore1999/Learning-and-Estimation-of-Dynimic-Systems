function [theta]=myRWLS_III(y,u,n,lambda)
    ...
    % Update the inverse of S(1)
    S1_inv = pinv(lambda*(phi1*phi1'));
    % Compute K(1)
    K1 =  S1_inv * phi1;
    ...
    for i=2:N
        ...
        % Update S(t)
        St_inv = S_inv_before/lambda - (S_inv_before*phit*phit'*S_inv_before)/(lambda*(lambda+phit'*S_inv_before*phit));        
        % Compute K(t)
        Kt =  St_inv * phit;
